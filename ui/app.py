...
from ui.animation import breathing_text, blinking_cursor, fade_in_window, set_enabled as _set_anim_enabled
...
class DiffGuardApp(ctk.CTk):
    ...
    def _build_body(self) -> None:
        ...
        self._build_review_module(self.module_container)
    ...
    def _on_module_selected(self, key: str) -> None:
        ...
        if key == "review":
            ...
            self._build_history_module(self.module_container)
        elif key == "permission":
            self._build_permission_module(self.module_container)
        elif key == "decision":
            self._build_decision_module(self.module_container)
        elif key == "settings":
            self._build_settings_module(self.module_container)
    ...
    def _animate_module_in(self) -> None:
        ...
        # 动画结束后复位透明度，避免残留
        widget.after(260, lambda: _safe_alpha(widget, 1.0))

    def _build_review_module(self, container: Any) -> None:
        """审查模块：风险仪表盘 + 左右分栏（文件/Diff + AI 报告）。"""
        ...
        # 左侧：文件列表 + Diff
        from ui.theme import frost_hi as _frost_hi, surface_border as _sborder

        left: ctk.CTkFrame = ctk.CTkFrame(
            main, corner_radius=12, fg_color="transparent",
            border_width=1, border_color=_sborder(True),
        )
        ...
        diff_scroll_y: ctk.CTkScrollbar = ctk.CTkScrollbar(diff_body, command=self.diff_textbox.yview)
        ...
        report_scroll_y: ctk.CTkScrollbar = ctk.CTkScrollbar(report_body, command=self.report_textbox.yview)
        report_scroll_y.grid(row=0, column=1, sticky="ns")
        self.report_textbox.configure(yscrollcommand=report_scroll_y.set)

        self._show_empty_guide(container)

    def _build_history_module(self, container: Any) -> None:
        """历史模块：内嵌历史列表。"""
        from ui.history_view import HistoryView

        container.grid_rowconfigure(0, weight=1)
        container.grid_columnconfigure(0, weight=1)
        view = HistoryView(container, app=self)
        view.grid(row=0, column=0, sticky="nsew")

    def _build_permission_module(self, container: Any) -> None:
        """权限模块：内嵌权限记录列表。"""
        from ui.permission_history_view import PermissionHistoryView

        container.grid_rowconfigure(0, weight=1)
        container.grid_columnconfigure(0, weight=1)
        view = PermissionHistoryView(container, app=self)
        view.grid(row=0, column=0, sticky="nsew")

    def _build_decision_module(self, container: Any) -> None:
        """决策模块：待决策状态 + 历史偏好。"""
        container.grid_rowconfigure(0, weight=1)
        container.grid_columnconfigure(0, weight=1)
        from ui.decision_view import DecisionView

        view = DecisionView(container, app=self)
        view.grid(row=0, column=0, sticky="nsew")

    def _build_settings_module(self, container: Any) -> None:
        """设置模块：复用设置对话框内容。"""
        self._open_settings()
    ...
    def _build_toolbar(self) -> None:
        ...
        self.save_button = _icon_button(tools, "保存", "保存到历史 (Ctrl+S)", self._save_history)
        _icon_button(tools, "导出", "导出报告 (Ctrl+E)", self._export_report)
        _icon_button(tools, "设置", "打开设置", self._open_settings)
        _icon_button(tools, "历史", "历史记录", self._open_history)
        _icon_button(tools, "权限", "权限记录", self._open_permission_history)
    ...
    def _toggle_nav_wide(self) -> None:
        """切换导航宽窄形态。"""
        self._nav_wide = not getattr(self, "_nav_wide", False)
        try:
    ...
    def _build_dashboard(self, container: Any = None) -> None:
        """构建顶部风险仪表盘卡：总分 + 等级色带 + 文件数 + 主要风险点。

        container: 目标容器；None 时用 self（向后兼容）。
        """
        parent: Any = container if container is not None else self
    ...
    def _show_empty_guide(self, parent: Any = None) -> None:
        """空状态提示：未载入 diff 时在左上角显示轻提示（不遮挡背景中心）。"""
        target: Any = parent if parent is not None else self
        from ui.theme import frost as _frost

        self._guide = ctk.CTkFrame(target, fg_color="transparent")
        self._guide.grid(row=0, column=0, sticky="nw", padx=4, pady=4)
        self._guide.grid_columnconfigure(0, weight=0)

        accent_color: str = accent_primary(self._accent, light=True)
        ctk.CTkLabel(
            self._guide,
            text="🛡 欢迎使用 DiffGuard",
            font=ctk.CTkFont(size=18, weight="bold"),
        ...
        ctk.CTkButton(
            ...
            fg_color=accent_color,
            hover_color=accent(self._accent, light=True)[1],
            width=120,
            height=30,
    ...
    def _update_decision_badge(self) -> None:
        """刷新导航"决策"项的待处理角标。"""
        try:
            if getattr(self, "nav", None) is not None:
                self.nav.set_badge("decision", 1 if self._decision_pending else 0)
        except Exception:
            pass
    ...
    def _on_paste_clicked(self) -> None:
        """从剪贴板读取内容并尝试加载。"""
        if not self._review_controls_ready():
            self._on_module_selected("review")
    ...
    def _update_risk_gauge(self) -> None:
        ...
        # 风险进度条（重构后可能未创建，存在才更新；仪表盘卡始终更新）
        rg = getattr(self, "risk_gauge", None)
        if rg is not None:
            try:
                rg.set_score(score, contributions)
            except Exception:
                pass
        try:
            # 仪表盘
    ...
    # AI 审查（流式）
    # ------------------------------------------------------------------
    def _review_controls_ready(self) -> bool:
        """判断审查模块控件是否有效存在（未被销毁）。"""
        for name in ("diff_textbox", "report_textbox", "file_list_frame"):
    ...
    def _start_review(self) -> None:
        ...
        # 审查相关控件只在"审查"模块存在；若当前在其它模块，先切回审查
        if not self._review_controls_ready():
            self._on_module_selected("review")
            self.update_idletasks()
        if self._analyzing:
        ...
        # 状态栏呼吸 + 报告区闪烁光标
        self._stop_breathing = breathing_text(self.status_label, "分析中")
        self._stop_cursor = blinking_cursor(self.report_textbox)
        self._clear_report()
    ...
    def _on_review_done(self) -> None:
        ...
        self.status_var.set("审查完成，可保存到历史" if self.report_textbox.get("1.0", "end").strip() else "审查完成，但未获得有效内容")
    ...
    # 设置
    # ------------------------------------------------------------------
    def _on_wizard_done(self, new_config: Config) -> None:
        """首次配置向导完成后：应用新配置。"""
        ...
        try:
            ...
            if new_config.auto_clipboard and self._watcher is None:
                self._start_clipboard_watching()
            if new_config.permission_monitor and self._permission_watcher is None:
                self._start_permission_watching()
        except Exception as exc:
            logger.debug("应用首次配置失败: {}", exc)
...
def _icon_button(
    parent: ctk.CTkFrame, label: str, tooltip: str, command: Callable[[], Any]
) -> ctk.CTkButton:
...
def _safe_alpha(widget: Any, value: float) -> None:
    """安全设置控件透明度（控件可能已销毁）。"""
    try:
